...
# 动量法会保存历史梯度并给予一定的权重，使其也参与到参数更新中
class Momentum:
    def __init__(self, lr=0.01, momentum=0.9):
        self.lr = lr  # 学习率
        self.momentum = momentum  # 历史梯度的权重
...
# RMSProp 均方根传播, 在 AdaGrad 基础上的改进
# 逐渐遗忘过去的梯度，采用指数移动加权平均，呈指数地减小过去梯度的尺度
class RMSProp:

    def __init__(self, lr=0.01, decay=0.99):
        self.lr = lr
        self.decay = decay  # 权重
        self.h = None  # 历史梯度平方和的指数移动加权平均
...
# Adam 自适应矩估计, 融合了 Momentum 和 AdaGrad 的方法
class Adam:
    # 初始化
